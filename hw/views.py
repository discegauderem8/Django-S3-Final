...
def get_orders(request, user_id):
    ...
    this_years_orders = orders.filter(created_at__gt=years_threshold)
    this_months_orders = orders.filter(created_at__gt=months_threshold)
    this_weeks_orders = orders.filter(created_at__gt=weeks_threshold)


    this_years_items = models.Item.objects.filter(order__in=this_years_orders)
